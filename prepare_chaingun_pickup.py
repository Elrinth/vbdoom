"""
Convert Powerups_002.png (chaingun pickup) to VB format for use as a pickup sprite.
Output: pickup_chaingun.c / pickup_chaingun.h (4x3 tiles = 32x24 pixels)
"""
import os
...
def main():
    ...
    for y in range(PX_H):
        for x in range(PX_W):
            ...
            for sy in range(4):
                for sx in range(4):
                    pp[x*4+sx, y*4+sy] = c
    prev.save(os.path.join(PREVIEW_DIR, "pickup_chaingun.png"))
    ...
    c_path = os.path.join(OUTPUT_DIR, "pickup_chaingun.c")
    with open(c_path, 'w') as f:
        f.write("/* Chaingun pickup sprite -- auto-generated */\n\n")
        f.write(f"const unsigned int pickup_chaingunTiles[{len(all_u32)}]"
    ...
    h_path = os.path.join(OUTPUT_DIR, "pickup_chaingun.h")
    with open(h_path, 'w') as f:
        f.write("#ifndef __PICKUP_CHAINGUN_H__\n#define __PICKUP_CHAINGUN_H__\n\n")
        f.write(f"extern const unsigned int pickup_chaingunTiles[{len(all_u32)}];\n\n")
        f.write("#endif\n")
